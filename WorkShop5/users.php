...
$conn = getConnection();
$sql = "SELECT u.id, u.firstName, u.lastName, u.email, u.status, u.last_login_datetime, p.name AS province 
        FROM users u 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $user = [
      ...
      'status' => isset($_POST['status']) ? $_POST['status'] : 'active',  // Aquí se asigna el valor de 'status'
  ];
    ...
    <form action="users.php" method="POST" class="mb-4">
      <div class="form-row">
        ...
        <div class="col">
         ...
         <!-- Aquí va el select para el campo "status" -->
         <div class="col">
    <select class="form-control" name="status" required>
        <option value="active" <?php echo (isset($editUser) && $editUser['status'] == 'active') ? 'selected' : ''; ?>>Active</option>
        <option value="inactive" <?php echo (isset($editUser) && $editUser['status'] == 'inactive') ? 'selected' : ''; ?>>Inactive</option>
    </select>
</div>
  ...
  <tbody>
    <?php while($user = mysqli_fetch_assoc($result)): ?>
      <tr>
        ...
        <td><?php echo $user['last_login_datetime'] ? date('Y-m-d H:i:s', strtotime($user['last_login_datetime'])) : 'Never'; ?></td>
        <td>
